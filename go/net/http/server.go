...
func (cr *connReader) backgroundRead() {
	...
	cr.lock()
	//fmt.Printf("backgroundRead cr.byteBuf:%v n:%v\n", string(cr.byteBuf[:]), n)
	if n == 1 {
		cr.hasByte = true
...
func (cr *connReader) abortPendingRead() {
	...
	cr.aborted = true
	fmt.Printf("abortPendingRead cr.inRead:%v\n", cr.inRead)
	cr.conn.rwc.SetReadDeadline(aLongTimeAgo)
	for cr.inRead {
		cr.cond.Wait()
	}
	fmt.Printf("abortPendingRead end\n")
...
func (w checkConnErrorWriter) Write(p []byte) (n int, err error) {
	//fmt.Printf("checkConnErrorWriter Write p:%v\n", string(p))
