...
import (
	...
	"io"
	"learn/http/go/bufio"
	"learn/http/go/net/http/httptrace"
...
func (t *Transport) tryPutIdleConn(pconn *persistConn) error {
	...
	if t.MaxIdleConns != 0 && t.idleLRU.len() > t.MaxIdleConns {
		//fmt.Printf("tryPutIdleConn t.idleLRU.len():%v\n", t.idleLRU.len())
...
func (t *Transport) queueForDial(w *wantConn) {
	...
	q.cleanFront()
	q.pushBack(w)
	fmt.Printf("queueForDial Maplen:%v len:%v w.key:%v\n", len(t.connsPerHostWait), q.len(), w.key)
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (pconn *persistConn, err error) {
	...
	//fmt.Printf("pconn.br:%v pconn.bw:%v\n", pconn.br.Size(), pconn.bw.Size())
...
func NewReaderSize(rd io.Reader) {
...
func (pc *persistConn) closeConnIfStillIdle() {
	fmt.Printf("closeConnIfStillIdle start\n")
	t := pc.t
	t.idleMu.Lock()
	defer t.idleMu.Unlock()
